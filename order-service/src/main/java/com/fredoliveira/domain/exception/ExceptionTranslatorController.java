...
public class ExceptionTranslatorController {
    ...
    public ErrorDTO processDataIntegrityViolation(DataIntegrityViolationException ex) {
        ...
        if (ex.getCause() instanceof ConstraintViolationException) {
            ...
            if (constraintName.contains("order_key")) {
                dto = ErrorDTO.builder()
                        .errorCode(ErrorDTO.ErrorCodes.ERR_DATA_INTEGRITY)
                        .message(Messages.getString(MessageKey.ORDER_ALREADY_EXISTS))
                        .build();
            } else if (constraintName.contains("order_item_key")) {
                dto = ErrorDTO.builder()
                        .errorCode(ErrorDTO.ErrorCodes.ERR_DATA_INTEGRITY)
                        .message(Messages.getString(MessageKey.ITEM_ORDER_ALREADY_EXISTS))
                        .build();
            } else {
                dto = ErrorDTO.builder()
                        .errorCode(ErrorDTO.ErrorCodes.ERR_DATA_INTEGRITY)
